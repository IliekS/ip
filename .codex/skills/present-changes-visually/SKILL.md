...
4. Run the bundled generator from the repository root. On Windows, use the
   available Python launcher (`py`) if `python3` is unavailable:

   ```bash
   py .codex/skills/present-changes-visually/scripts/generate-split-view-diff.py \
